...
MIN_PROFIT = float(os.getenv("V31_4H_REVERSE_MIN_PROFIT", "20000000"))
MIN_ROI = float(os.getenv("V31_4H_REVERSE_MIN_ROI", "0.10"))
LIVE_LIMIT = int(os.getenv("V31_4H_REVERSE_LIVE_LIMIT", "160"))
TOP = int(os.getenv("V31_4H_REVERSE_TOP", "100"))
HAUL_BASE = float(os.getenv("V31_4H_REVERSE_HAUL_BASE", "10000000"))
HAUL_PER_M3 = float(os.getenv("V31_4H_REVERSE_HAUL_ISK_PER_M3", "500"))
RESULT = LATEST / "v31_jita_to_four_h.csv"
REPORT = LATEST / "v31_jita_to_four_h.md"


def _net_after_base(q):
    if not q:
        return None
    out = dict(q)
    out["haul_cost"] = float(out.get("haul_cost", 0) or 0) + HAUL_BASE
    out["net_profit"] = float(out.get("net_profit", 0) or 0) - HAUL_BASE
...
def _fixed_qty_quote(asks, bids, qty, unit_m3):
    ...
    if not source.complete or not dest.complete:
        return None
    haul = HAUL_BASE + max(0.0, unit_m3) * qty * HAUL_PER_M3
...
def main():
    ...
    print("Opportunity Engine V3.1: Jita -> 4-H reverse market arbitrage")
    ...
    if not four_h_buys:
        ...
        REPORT.write_text("# V3.1 Jita to 4-H\n\nNo 4-H buy orders.\n", encoding="utf-8")
        return
    ...
    for tid, bids in four_h_buys.items():
        ...
        if not q or q["net_profit"] < MIN_PROFIT * 0.5:
            continue
    ...
    print(f"V3.1 reverse broad={len(broad)}")
    if not ids:
        pd.DataFrame().to_csv(RESULT, index=False)
        REPORT.write_text("# V3.1 Jita to 4-H\n\nNo broad candidates.\n", encoding="utf-8")
        return
    ...
    for r in broad:
        ...
        if stress_source and stress_dest:
            ...
            stress_profit = -float(q["source_cost"])
    ...
    REPORT.write_text(
        "\n".join(
            [
                "# V3.1 Jita to 4-H reverse arbitrage",
                "",
                f"- 4-H orders: {len(structure_rows)}",
                f"- Jita snapshot: {m_modified}",
                f"- Live revalidation: {live_at}",
                f"- Logistics reserve: {HAUL_BASE:,.0f} ISK base + {HAUL_PER_M3:,.0f} ISK/m3",
                f"- Final opportunities: {len(final)}",
                "- Stress holds the planned quantity fixed and removes each side's best level separately.",
            ]
        )
        + "\n",
        encoding="utf-8",
    )
    print(f"V3.1 reverse done: opportunities={len(final)}")
