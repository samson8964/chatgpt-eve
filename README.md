...
1. `runner_buy_only.py`：BPC 制造扫描；合同 >50亿、SKIN/SKINR、旗舰船体和建筑物本体蓝图在制造预筛前就剔除。粗筛后最多保留 **500 份**进入昂贵的精算阶段。成品收入只来自 Jita 买单；Broker 和改价预留为 0。
2. `bpc_contract_benchmark_buy_only.py`：BPC 每流程挂牌比较；建立同类比较池前同样剔除 >50亿、SKIN/SKINR、旗舰船体和建筑物本体蓝图，防止它们进入“蓝图价值”邮件或污染价格基准。
3. `apply_buy_only_bpc_policy.py`：对全部 BPC 输出做第二层价格、SKIN、旗舰船体、建筑物本体保护，并按 Buy-Only 模型复核制造净利润。
...
BPC 制造的成品收入端使用当前 Jita 买单深度。新版成本口径：
...
制造粗筛后最多选择 **500 份最优候选**进行精算。进入这 500 个名额之前就剔除：

- 航母、无畏、力辅、工业旗舰、枪骑无畏、超级航母、泰坦、货舰、跳跃货舰等旗舰/超大型船体蓝图；
- Citadel、Engineering Complex、Refinery、FLEX、Control Tower、Assembly Array、Mobile Laboratory、主权/轨道基础设施等建筑物本体蓝图；
- SKIN/SKINR 与皮肤制作相关蓝图；
- 合同价 >50亿的蓝图。
...
蓝图价差比较池也执行与制造通道相同的品类排除：**旗舰船体、建筑物本体、SKIN/SKINR、合同价 >50亿** 全部在建立同类价格基准之前剔除。因此 Hel、Nyx 等蓝图不会再以 `【蓝图-价值】` 形式进入通用 BPC 邮件。
...
| 工作流 | 调度 | 说明 |
|---|---|---|
| `EVE arbitrage scans` | 每小时 07/17/27/37/47/57 分提供触发机会，25 分钟门控后通常约每 30 分钟完整扫描一次 | 60 分钟上限 |
