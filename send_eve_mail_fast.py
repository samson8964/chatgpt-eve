...
MAIL_TOP = int(os.getenv("MAIL_TOP", "5"))
...
def main():
    ...
    if not df.empty:
        ...
        df = df.loc[keep].sort_values(["opportunity_score", "net_profit"], ascending=[False, False]).head(MAIL_TOP)
    ...
    if df.empty:
        subject = f"BPC扫描完成 {stamp} · 暂无强机会"
        body = (
            f"<b>EVE BPC 快速扫描完成</b><br>{stamp}<br><br>"
            f"本轮主榜 {total} 个；没有同时满足 净利≥{fmt_isk(MAIL_MIN_PROFIT)}、ROI≥{MAIL_MIN_ROI*100:.0f}%、买盘容量≥1批 的机会。<br><br>"
            "这封状态邮件也表示 GitHub → LadyGuaGua → MikeChong 推送链路正常。"
        )
    else:
        subject = f"BPC捡漏 {stamp} · {len(df)}个强机会"
        ...
        for i, (_, r) in enumerate(df.iterrows(), 1):
            ...
            try:
                ...
                pass
            parts.append("<br><br>")
        parts.append("下单前再核对合同仍存在、BPC runs/ME/TE、Jita买盘。")
        body = "".join(parts)
